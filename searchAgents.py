...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextPos = (nextx, nexty)
                    # อัปเดตสถานะการแวะมุม
                visitedList = list(state[1])
                for i in range(len(self.corners)):
                    if nextPos == self.corners[i]:
                        visitedList[i] = True
                    
                nextState = (nextPos, tuple(visitedList))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for path in itertools.permutations(unvisited):
        dist = util.manhattanDistance(currentPos, path[0])
        for i in range(len(path) - 1):
            dist += util.manhattanDistance(path[i], path[i+1])
        
        if dist < min_path_sum:
            min_path_sum = dist
